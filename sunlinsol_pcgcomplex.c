                       ...
                       N_Vector x, N_Vector b, sunrealtype delta)
{
  ...
  suncomplextype alpha, beta, rz, rz_old;
  N_Vector r, p, z, Ap, w;
  ...
  *nli      = 0;
  ...
  else
  {
    ...
    N_VLinearSum_SComplex(ONE, b, -ONE, r, r);
  }

  /* Set rho to scaled L2 norm of r, and return if small */
  if (UseScaling)
  {
    N_VProd_SComplex(r, w, Ap);
  }
  else
  {
    N_VScale_SComplex(ONE, r, Ap);
  }
  rho = (sunrealtype)N_VDotProd_SComplex(Ap, Ap);
  *res_norm = r0_norm = rho = SUNRsqrt(rho);
  ...
  for (l = 0; l < l_max; l++)
  {
    ...
    if (l == 0 && *zeroguess)
    {
      N_VScale_SComplex(alpha, p, x);
    }
    else
    {
      N_VLinearSum_SComplex(ONE, x, alpha, p, x);
    }

    /* Update r = r - alpha*Ap */
    N_VLinearSum_SComplex(ONE, r, -alpha, Ap, r);
    ...
    *res_norm = rho = SUNRsqrt(rho);
    ...
    /* Update p = z + beta*p */
    N_VLinearSum_SComplex(ONE, z, beta, p, p);
  }
                              ...
                              long int* leniwLS)
{
  ...
  N_VSpace_SComplex(PCG_CONTENT(S)->r, &lrw1, &liw1);
  *lenrwLS = 1 + lrw1 * 4;
  *leniwLS = 4 + liw1 * 4;
  return SUN_SUCCESS;
}
...
SUNErrCode SUNLinSolFree_PCG(SUNLinearSolver S)
{
  ...
  if (S->content)
  {
    ...
    if (PCG_CONTENT(S)->r)
    {
      N_VDestroy_SComplex(PCG_CONTENT(S)->r);
      PCG_CONTENT(S)->r = NULL;
    }
    if (PCG_CONTENT(S)->p)
    {
      N_VDestroy_SComplex(PCG_CONTENT(S)->p);
      PCG_CONTENT(S)->p = NULL;
    }
    if (PCG_CONTENT(S)->z)
    {
      N_VDestroy_SComplex(PCG_CONTENT(S)->z);
      PCG_CONTENT(S)->z = NULL;
    }
    if (PCG_CONTENT(S)->Ap)
    {
      N_VDestroy_SComplex(PCG_CONTENT(S)->Ap);
